...
public class WikiMediaProducer {
    public static void main(String[] args) throws StreamException, InterruptedException {
        ...
        HttpConnectStrategy httpConnectStrategy = ConnectStrategy.http(URI.create(url)).connectTimeout(3, TimeUnit.SECONDS);
        BackgroundEventSource eventSource = new BackgroundEventSource.Builder(
                eventHandler,
                new EventSource.Builder(httpConnectStrategy)
        ).build();
